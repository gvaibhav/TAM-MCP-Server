...
export class BlsService implements DataSourceService {
    ...
    constructor(apiKey?: string) {
        ...
        if (!this.apiKey) {
            console.log("ℹ️  BLS: API key not configured - using public access with limited rate limits (set BLS_API_KEY to enable full access)");
        }
    }

    private async fetchApiData(seriesIds: string[], startYear?: string, endYear?: string, options: any = {}): Promise<any> {
        try {
            const data: any = {
                seriesid: seriesIds,
    ...
    async fetchSeriesData(seriesIds: string[], startYear?: string, endYear?: string, options: any = {}): Promise<any> {
        logger.info('BlsService.fetchSeriesData called', { seriesIds, startYear, endYear, options });
    ...
    async fetchMarketSize(industryId: string, region?: string): Promise<any> {
        try {
            ...
            const data = await this.fetchSeriesData([seriesId], undefined, undefined, {});

            if (!data?.Results?.series?.[0]?.data || data.Results.series[0].data.length === 0) {
                return null;
            }
    ...
    async fetchIndustryData(seriesIds: string[], startYear?: string, endYear?: string, options: any = {}): Promise<any> {
        logger.info('BlsService.fetchIndustryData called', { seriesIds, startYear, endYear, options });
