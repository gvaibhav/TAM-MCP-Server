...
export class CensusService implements DataSourceService {
    ...
    constructor(apiKey?: string) {
        ...
        if (!this.apiKey) {
            console.log("ℹ️  Census: API key not configured - using public access with limited rate limits (set CENSUS_API_KEY to enable full access)");
        }
    }
    ...
    async getData(dataset: string, variables: string, geography: string, params: any = {}): Promise<any> {
        logger.info('CensusService.getData called', { dataset, variables, geography, params });
    ...
    async fetchIndustryData(naicsCode: string, geography: string = 'state:*', params: any = {}): Promise<any> {
        logger.info('CensusService.fetchIndustryData called', { naicsCode, geography, params });
        ...
        try {
            ...
            const data = await this.getData(censusApi.cbpDataset, variables, geography, queryParams);
    ...
    async fetchMarketSize(naicsCode: string, geography: string = 'us:*', measure: string = 'EMP'): Promise<any> {
        try {
