...
export class ImfService extends APIService implements DataSourceService {
  ...
  async fetchImfDataset(
    ...
    try {
      ...
      if (error.isAxiosError && error.response) {
        ...
        throw new Error(
          `IMF API Error: ${status} - ${JSON.stringify(errorData)}`,
        );
      }
    ...
    try {
      ...
      if (data && Array.isArray(data) && data.length > 0) {
        ...
        for (const record of data) {
          ...
          if (
            !isNaN(recordYear) &&
            (isNaN(latestYear) || recordYear > latestYear)
          ) {
            latestRecord = record;
  ...
  async searchDataset(searchQuery: string, params?: any): Promise<any> {
    logger.warn(
      "ImfService.searchDataset is a placeholder and needs specific implementation.",
      { searchQuery, params },
    );
    return {
  ...
  async fetchIndustryData(...args: any[]): Promise<any> {
    ...
    try {
      ...
      return await this.fetchImfDataset(
        dataflowId,
        key,
        startPeriod,
        endPeriod,
